...
async def ban_user(chat_id, i, rights):
    try:
        await WWWL5(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@WWWL5.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@WWWL5.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@WWWL5.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
@WWWL5.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await WWWL5(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in WWWL5.iter_participants(chat_id):
        ...
        await WWWL5.send_message(chat_id, usrtxt)
...
@WWWL5.ar_cmd(pattern="الغاء التفليش", groups_only=True)
