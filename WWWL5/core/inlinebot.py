...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**WWWL5"):
            ...
            builder = event.builder
            troll = os.path.join("./WWWL5", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./WWWL5", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./WWWL5", "hide.txt")
            try:
            ...
            result = builder.article(
                title="WWWL5 Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب جاكو **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل جاكو من تنصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝙅𝘼𝙆𝙊𝙊 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/mvhmed)",
...
@WWWL5.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@WWWL5.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@WWWL5.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@WWWL5.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
