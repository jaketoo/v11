...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await WWWL5(UnblockRequest("@sourcejakoo"))
        await WWWL5(
            UpdateNotifySettingsRequest(
                peer="t.me/sourcejakoo",
                settings=InputPeerNotifySettings(mute_until=2**31 - 1),
            )
        )
        await WWWL5.edit_folder("@sourcejakoo", folder=1)  # عمل ارشيف للبوت
        await WWWL5(UnblockRequest("@sourcejakoo"))
        await WWWL5(JoinChannelRequest("@sourcejakoo"))
        await WWWL5(JoinChannelRequest("@sourcejakoo"))
        await WWWL5(JoinChannelRequest("@sourcejakoo"))
        await WWWL5(JoinChannelRequest("@sourcejakoo"))
        await WWWL5(JoinChannelRequest("@sourcejakoo"))
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await WWWL5.send_message("@sourcejakoo", "/start")
            await asyncio.sleep(1)
            await WWWL5.send_message(
                "@sourcejakoo",
                "⎊ تم بنجاح تشغيل سورس جاكو 🕷",
            )
            await asyncio.sleep(1)
        ...
        try:
            if BOTLOG:
                await WWWL5.tgbot.send_message(
                    BOTLOG_CHATID,
                    "**تم تشغيل سورس جاكو بنجاح**\n➖➖➖➖➖➖➖➖➖➖\n**قناة السورس: @mvhmed**\n**قناة الكلايش: @mvhmed**\n**مطور السورس: @mvhmed**\n➖➖➖➖➖➖➖➖➖➖\n**لعرض الاوامر ارسل `.الاوامر`**",
                    buttons=[
                        (Button.url("كروب المساعدة", "https://t.me/mvhmed"),)
                    ],
                )
        except Exception as e:
            LOGS.error(e)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "جروب اشعارات جاكو", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
        addgvar("PRIVATE_GROUP_BOT_API_ID", groupid)
        print("تم انشاء جروب الاشعارات بنجاح")
        flag = True
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "جروب تخزين جاكو", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
