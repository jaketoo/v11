...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس جاكو " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس جاكو"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس جاكو أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/jaketo/11"
    os.chdir("/app")
    try:
