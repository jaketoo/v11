...
async def on_new_private_message(event):
    ...
    if event.chat_id == 6080661382:
        await event.client.send_message(chat, "⪼ اهـلا مـبرمج الـسورس الغـالي جاكو اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰")
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق جاكو
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@WWWL5.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق جاكو
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق جاكو
...
@WWWL5.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@WWWL5.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية جاكو.txt",
